...
import { WeChatClawBotSchema } from "../src/compositions/WeChatClawBot/schema";

const SCENE_COUNT = 7;
const COMPOSITION_PREFIX = "wechatclawbot";
const defaultScripts = WeChatClawBotSchema.parse({}).voiceoverScripts;
...
async function syncSubtitles() {
  console.log("🔄 开始同步 WeChatClawBot 字幕和配音...\n");
  ...
  for (let index = 0; index < audioInfos.length; index++) {
    ...
    if (fs.existsSync(vttFile)) {
      const cues = parseVtt(fs.readFileSync(vttFile, "utf-8"));
      ...
      cues.forEach((cue) => {
        ...
        words.push(...buildWords(cue.text, cueStartFrame, cueEndFrame));
      });
    } else {
  ...
  console.log(`\n✨ WeChatClawBot 字幕同步完成，总时长 ${(totalDuration / fps).toFixed(1)} 秒`);
}
