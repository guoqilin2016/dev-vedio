...
import { AgentSkillsSchema } from "../src/compositions/AgentSkills/schema";

const SCENE_COUNT = 7;
const COMPOSITION_PREFIX = "agentskills";
const defaultScripts = AgentSkillsSchema.parse({}).voiceoverScripts;
...
function buildWords(text: string, startFrame: number, endFrame: number): SubtitleWord[] {
  ...
  const visibleChars =
    chars.filter((c) => !/[\s，。！？、：；,.!?]/.test(c)).length || chars.length;
  ...
  for (const char of chars) {
    ...
    currentWord += char;
    if (currentWord.length >= 4) flushWord();
  }
...
async function syncSubtitles() {
  console.log("🔄 开始同步 AgentSkills 字幕和配音...\n");
  ...
  for (let index = 0; index < audioInfos.length; index++) {
    ...
    if (fs.existsSync(vttFile)) {
      ...
      cues.forEach((cue) => {
        ...
        if (cueEndFrame < cueStartFrame) cueEndFrame = cueStartFrame + 2;
  ...
  console.log(`\n✨ AgentSkills 字幕同步完成，总时长 ${(totalDuration / fps).toFixed(1)} 秒`);
}
