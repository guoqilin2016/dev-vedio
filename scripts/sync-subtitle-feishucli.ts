...
import { FeishuCLISchema } from "../src/compositions/FeishuCLI/schema";

const SCENE_COUNT = 7;
const COMPOSITION_PREFIX = "feishucli";
const defaultScripts = FeishuCLISchema.parse({}).voiceoverScripts;
...
async function syncSubtitles() {
  console.log("🔄 开始同步 FeishuCLI 字幕和配音...\n");
  ...
  console.log(`\n✨ FeishuCLI 字幕同步完成，总时长 ${(totalDuration / fps).toFixed(1)} 秒`);
}
